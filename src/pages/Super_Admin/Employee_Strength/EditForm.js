...
function EditForm(props) {
  ...
  const HandleStatus = (val) => {
    ...
    result.map((item, key) => {
      setSelectedUser({
        number_of_employee: selectedUser.number_of_employee,
        status: val,
      });
    });
  };
  ...
  const OnSubmitForm = (values, props) => {
    ...
    var raw = JSON.stringify({
      "number_of_employee": values.number_of_employee, "status": values.status, created_on: "2021/02/02",
      created_by: user_id,
      updated_by: user_id,
    ...
    fetch(APIUrl + `/employee/${comid}`, requestOptions)
      ...
      .then((resData) => {
        ...
        if (resData.status == 200) {
          setConfirmDialog({
            isOpen: true,
            title: 'Alert',
            subTitle: "Employee Strength Updated Successfully",
          })
              ...
              <div className="container">
                <div className="row">
                  <div className="col s10 m6 l6">
                    <h5 className="breadcrumbs-title mt-0 mb-0"><span>{showEdit ? "Edit" : "View"} Employee Strength</span></h5>
                  </div>
                </div>
              </div>
            </div>
